...
- Preserve source atomicity in `App.jsx`: each create/edit must leave a syntactically valid, renderable file with defined dependencies and a complete root render. Never save unclosed JSX or half a component as a progress checkpoint. Once the frame works, enrich it with coherent edits instead of replacing the whole app for every detail.
